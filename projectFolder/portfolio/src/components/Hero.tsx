      ...
      <div className="relative z-10 container mx-auto px-6 pb-16 pt-10">
        <div className="grid gap-12 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
          <div>
            <div
              ref={badgeRef}
              className="mb-7 hidden w-fit items-center gap-2 rounded-full border border-primary/30 bg-background/40 px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-primary/85 sm:inline-flex"
            >
              <Sparkles className="h-3.5 w-3.5" />
              AI delivery lab for ambitious brands
            </div>

            <h1 className="max-w-4xl text-[2.4rem] font-extrabold leading-[1.04] text-foreground sm:text-5xl lg:text-7xl">
            ...
            <p
              ref={subtitleRef}
              className="mt-8 max-w-2xl text-base leading-relaxed text-muted-foreground sm:text-xl"
            >
            ...
            >
              {proofs.map((proof) => (
                <div
                  key={proof.label}
                  className="section-ring rounded-2xl bg-background/35 p-4"
                >
                  <p className="text-2xl font-extrabold text-foreground sm:text-3xl">
                    {proof.value}
                  </p>
                  <p className="mt-1 text-sm text-muted-foreground">{proof.label}</p>
                </div>
              ))}
            </motion.div>
          </div>
          ...
          <motion.aside
            ...
            className="glass-card section-ring rounded-3xl p-7"
          >
